...
public:
	...
	[[nodiscard]] auto asVectorOfScalars() const {
...
inline void accuTest() {
	auto accu1 = Avx2StatsAccumulator();
	auto accu2 = Avx2StatsAccumulator();

	std::random_device rd;
	std::mt19937 e2(rd());
	std::normal_distribution<> dist(100, 50);

	auto data = std::vector<double4>(1000000);
	for (auto& item : data) {
		...
		// 	}
		// 	else {
		// 		item = { INFINITY, INFINITY, INFINITY, INFINITY };
		// 	}
		// }
		// else {
		// 	item = { dist(e2), dist(e2), dist(e2), dist(e2) };
		// }
		item = {dist(e2), dist(e2), dist(e2), dist(e2)};
	}

	// data[0] = { NAN, NAN, NAN, NAN };

	for (const auto item : data) {
		accu1.pushWithFiltering(item);
		accu2.push(item);
	}

	auto res1 = accu1.asScalar();
	auto res2 = accu2.asScalar();
